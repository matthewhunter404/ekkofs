    ...
    try {
      ...
      if (error instanceof QueryFailedError) {
        ...
        if (message.includes('violates unique constraint')) {
          if (detail.includes('parentstructureid')) {
            throw new HttpException(
              'Duplicate Parent Structure Assignment, each parent structure can only have one child',
              HttpStatus.BAD_REQUEST,
            );
          }
          if (detail.includes('name') && detail.includes('already exists')) {
            throw new HttpException(
              'Duplicate Name, a structure with that name already exists',
              HttpStatus.BAD_REQUEST,
            );
          }
        }
      ...
      throw new Error(
        'An unexpected error occurred while creating the structure',
      );
    }
